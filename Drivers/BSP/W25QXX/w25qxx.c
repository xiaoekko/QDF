...
#include <stdlib.h>  // 包含stdlib.h以使用malloc和free函数


History HistoryData;
uint8_t HistoryBuffWrite[20];
uint8_t HistoryBuffRead[20];
uint32_t FlashLocate;

uint32_t maxnumber;
uint32_t number=1;

uint16_t W25QXX_TYPE=W25Q64;	// 默认W25Q64
...
{ 
	...
	SPI1_Init();		   			// 初始化SPI
	SPI1_SetSpeed(SPI_BAUDRATEPRESCALER_2);		// 设置SPI速度 
...
{  
	W25QXX_CS_LOW();								// 使能器件   
	SPI1_ReadWriteByte(W25X_WriteDisable);	 // 发送写禁止    
	W25QXX_CS_HIGH();								// 取消片选     	      
}
...
void W25QXX_Write_Page(uint8_t* pBuffer,uint32_t WriteAddr,uint16_t NumByteToWrite)
{
	...
	for(i=0;i<NumByteToWrite;i++)SPI1_ReadWriteByte(pBuffer[i]);// 循环写入  
	W25QXX_CS_HIGH();												// 取消片选 
...
{ 			 
	...
	{   
	 	...
	 	else // NumByteToWrite>pageremain
		{
			...
			if(NumByteToWrite>256)pageremain=256; // 下一页要写的数据量
			else pageremain=NumByteToWrite; 	  // 不够256字节了
		}
	};	    
} 
...
{ 
	...
	uint16_t secremain;	   
 	uint16_t i;    
	uint8_t * W25QXX_BUF;	  
   	W25QXX_BUF=(uint8_t*)malloc(4096);		 // 开辟4096字节缓存
	if(!W25QXX_BUF)return;						// 内存申请失败
	...
	{	
		...
		}else W25QXX_Write_NoCheck(pBuffer,WriteAddr,secremain);// 写入已经擦除了的,直接写入扇区剩余区间. 				
